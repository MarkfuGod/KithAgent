...
The `desktop/` folder contains a Mac-first Electron companion for the same local daemon. It gives non-developer users a Jarvis-style surface without asking them to run curl or remember syscall names.
...
- exposes **Ask Jarvis**, **About Me**, **Memories**, **Sources & Privacy**, and **Advanced** tabs
...
| Syscall | Purpose |
| --- | --- |
| `assistant.chat` | Ask Jarvis using profile, reports, recent files, and RAG evidence |
...
`desktop/` 目录是基于 Electron 的 Mac 桌面伴侣，用同一个本地 daemon，但给普通用户一个 Jarvis 风格界面，不需要记 curl 或 syscall 名字。
...
- 提供 **Ask Jarvis**、**About Me**、**Memories**、**Sources & Privacy**、**Advanced** 五个标签
...
| Syscall | 用途 |
| --- | --- |
| `assistant.chat` | 用画像、报告、最近文件和 RAG evidence 向 Jarvis 提问 |
