...
### Backend-first CLI focus

KithAgent is intentionally backend-first: the CLI starts and observes the local daemon, runs First Insight, checks health, and exposes the same memory/RAG/profile surface that other agents call. It is not trying to be a Hermes Agent-style general TUI/chat agent. Hermes is strongest as a self-improving interactive agent shell with slash commands, sessions, skills, gateways, and model switching; Kith's center of gravity is the always-on local memory backend that lets those agents know the real user.
...
# Open the command center / health check
agent-sys
agent-sys doctor

# Seed the first correctable profile from the CLI
agent-sys first-insight

# Follow daemon logs in a separate terminal when you want details
agent-sys logs -f
...
CLI path. Interactive mode shows numbered presets across personal, work, study, life, and creative contexts; choose `Other / custom` when the presets miss.

```bash
agent-sys first-insight
agent-sys first-insight --role "developer, creator" --goal "ship Kith CLI" --focus "backend experience" --no-browser-history
agent-sys first-insight --json --yes --no-browser-history
```
...
agent-sys status                    # readable backend health (scan, memory, scheduler, LLM, RAG)
agent-sys status --json             # raw status for scripts
agent-sys logs                      # show recent daemon logs
agent-sys logs -f                   # follow logs in real time
agent-sys doctor                    # environment + daemon health checks
agent-sys statusline                # compact one-line status for shell prompts
...
KithAgent 的核心是后端优先：CLI 负责启动和观察本地 daemon、运行 First Insight、检查健康状态，并把同一套 memory/RAG/profile 能力暴露给其他 agent 调用。它不是要做成 Hermes Agent 那种通用 TUI/聊天 agent。Hermes 更强在自进化交互 shell、slash commands、sessions、skills、gateway 和模型切换；Kith 的中心是常驻本地记忆后端，让这些 agent 能真正理解用户。
...
CLI 路径。交互模式会展示覆盖个人、工作、学习、生活、创作的编号选项；如果没有合适项，选择 `Other / custom` 后自己填写。
...
agent-sys status                    # 可读后端健康状态（扫描、记忆、调度、LLM、RAG）
agent-sys status --json             # 给脚本使用的原始状态
agent-sys logs                      # 查看最近 daemon 日志
agent-sys logs -f                   # 实时跟随日志
agent-sys doctor                    # 环境 + daemon 健康检查
agent-sys statusline                # 给 shell prompt 用的一行状态
